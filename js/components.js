(function () {
  ...
  function footCol(title, items) {
    return '<div><div class="footer-heading">' + title + '</div><ul class="footer-links">' + items.map(function (item) { return '<li>' + link(item[0], item[1]) + '</li>'; }).join('') + '</ul></div>';
  }
  ...
  if (nav) {
    nav.innerHTML =
      ...
      link('pages/roller-replacement.html', 'Roller Replacement') + link('pages/track-repair.html', 'Track Repair') + link('pages/impact-glass.html', 'Impact Glass') + link('pages/lock-security.html', 'Lock & Security') + link('pages/window-repair.html', 'Window Repair') + link('pages/hoa-commercial.html', 'HOA & Commercial') + '</div></li>' +
      '<li class="has-drop"><a href="#" aria-label="Service regions">Regions</a><div class="drop-menu">' +
      link('pages/miami-dade.html', 'Miami-Dade County') + link('pages/broward.html', 'Broward County') + link('pages/palm-beach.html', 'Palm Beach County') + link('pages/florida-keys.html', 'Florida Keys') + '</div></li>' +
      '<li class="has-mega"><a href="#" aria-label="Service cities">Cities</a><div class="mega-menu">' +
      city('homestead', 'Homestead', 'Miami-Dade') + city('miami', 'Miami', 'Miami-Dade') + city('miami-beach', 'Miami Beach', 'Miami-Dade') + city('hialeah', 'Hialeah', 'Miami-Dade') + city('doral', 'Doral', 'Miami-Dade') + city('coral-gables', 'Coral Gables', 'Miami-Dade') + city('kendall', 'Kendall', 'Miami-Dade') + city('aventura', 'Aventura', 'Miami-Dade') + city('hollywood', 'Hollywood', 'Broward') + city('fort-lauderdale', 'Fort Lauderdale', 'Broward') + city('pembroke-pines', 'Pembroke Pines', 'Broward') + city('weston', 'Weston', 'Broward') + city('coral-springs', 'Coral Springs', 'Broward') + city('boca-raton', 'Boca Raton', 'Palm Beach') + city('delray-beach', 'Delray Beach', 'Palm Beach') + city('west-palm-beach', 'West Palm Beach', 'Palm Beach') + city('palm-beach-gardens', 'Palm Beach Gardens', 'Palm Beach') + city('jupiter', 'Jupiter', 'Palm Beach') + '</div></li>' +
      ...
      '</ul>' +
      '<div class="nav-right"><a href="tel:' + PHONE_TEL + '" class="nav-phone"><span aria-hidden="true">&#128222;</span> ' + PHONE_DISPLAY + '</a>' +
  ...
  if (footer) {
    footer.innerHTML = '<div class="container"><div class="footer-grid">' +
      '<div><a href="' + ROOT + 'index.html" aria-label="CoastSlide home">' + brand() + '</a><p class="footer-desc">South Florida bilingual sliding door and window repair. Miami-Dade NOA certified. Licensed FL contractor. Homestead to Jupiter.</p><div class="footer-socials"><a href="#" class="fsoc" aria-label="CoastSlide on Facebook">f</a><a href="#" class="fsoc" aria-label="CoastSlide on Instagram">in</a><a href="#" class="fsoc" aria-label="CoastSlide on YouTube">yt</a><a href="#" class="fsoc" aria-label="CoastSlide Google reviews">g</a></div></div>' +
  ...
  if (trust) trust.outerHTML = '<div class="trust-bar"><div class="trust-inner"><div class="trust-item"><span class="ti" aria-hidden="true">&#127942;</span> Licensed & Insured</div><div class="trust-item"><span class="ti" aria-hidden="true">&#9889;</span> Same-Day Service</div><div class="trust-item"><span class="ti" aria-hidden="true">&#128172;</span> English &amp; Espa&ntilde;ol</div><div class="trust-item"><span class="ti" aria-hidden="true">&#10003;</span> Miami-Dade NOA</div><div class="trust-item"><span class="ti" aria-hidden="true">&#128274;</span> Lifetime Warranty</div><div class="trust-item"><span class="ti" aria-hidden="true">&#128656;</span> Parts on Every Truck</div></div></div>';

  var cta = document.getElementById('cta-section');
  if (cta) cta.outerHTML = '<section class="cta-section"><div class="cta-wave"><svg viewBox="0 0 1440 60" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" aria-hidden="true"><path d="M0,0 C360,60 1080,0 1440,40 L1440,0 L0,0 Z" fill="white"/></svg></div><div class="container"><div class="cta-inner"><div class="chip" style="background:rgba(255,255,255,.15);border-color:rgba(255,255,255,.25);color:#fff;margin:0 auto 20px"><span class="dot"></span> Free Estimate - Same Day Available</div><h2 class="title white center">Your Door Fixed.<br/><em>Today.</em></h2><p class="lead center" style="color:rgba(255,255,255,.92);margin:16px auto 0">Flat-rate pricing. Bilingual. Licensed & insured. No surprises.</p><div class="cta-phones"><a href="tel:' + PHONE_TEL + '" class="cta-phone"><div class="cta-phone-area">One Number for South Florida</div><div class="cta-phone-num">' + PHONE_DISPLAY + '</div></a></div><p class="cta-note">Mon-Sat 7am-8pm &middot; Emergency 24/7 &middot; Text OK &middot; Se Habla Espa&ntilde;ol</p></div></div></section>';
  ...
  function normalizePhones() {
    ...
    var walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, { acceptNode: function (node) { var parent = node.parentElement; return !parent || /^(SCRIPT|STYLE|NOSCRIPT)$/i.test(parent.tagName) ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT; } });
    var current;
  ...
  function prepareForms() {
    document.querySelectorAll('form').forEach(function (form, formIndex) {
      ...
      form.querySelectorAll('.form-field').forEach(function (field, fieldIndex) {
        ...
        var hint = ((label && label.textContent) || '') + ' ' + (control.placeholder || '');
  ...
  function ensureMainLandmark() {
    ...
    if (firstAnchor && firstAnchor.nextSibling) body.insertBefore(main, firstAnchor.nextSibling); else body.insertBefore(main, body.firstChild);
    var node = main.nextSibling;
    while (node && node !== footerNode) { var next = node.nextSibling; if (node.nodeType !== 1 || (node.id !== 'mob-nav' && !node.classList.contains('mobile-nav') && !node.classList.contains('wa-float'))) main.appendChild(node); node = next; }
  }
  window.csToggleMobile = function () {
    var menu = document.getElementById('mob-nav'); var btn = document.getElementById('burger'); if (!menu) return;
    menu.classList.toggle('open'); if (btn) { btn.textContent = menu.classList.contains('open') ? 'X' : '\u2630'; btn.setAttribute('aria-label', menu.classList.contains('open') ? 'Close menu' : 'Open menu'); }
  };
  window.showReg = function (id) {
    ...
    var panel = document.getElementById('rp-' + id); if (panel) panel.classList.add('active');
    var tabs = document.querySelectorAll('.rtab'); var index = ['miami', 'broward', 'palm', 'keys'].indexOf(id); if (tabs[index]) tabs[index].classList.add('active');
  };
  document.addEventListener('DOMContentLoaded', function () {
    ensureMainLandmark(); normalizePhones(); prepareForms(); wireContactForms();
    document.querySelectorAll('.faq-item').forEach(function (item) { item.addEventListener('click', function () { var open = item.classList.contains('open'); document.querySelectorAll('.faq-item').forEach(function (node) { node.classList.remove('open'); }); if (!open) item.classList.add('open'); }); });
    if (window.IntersectionObserver) { var io = new IntersectionObserver(function (entries) { entries.forEach(function (entry) { if (entry.isIntersecting) { entry.target.classList.add('visible'); io.unobserve(entry.target); } }); }, { threshold: 0.08 }); document.querySelectorAll('.reveal').forEach(function (el) { io.observe(el); }); } else { document.querySelectorAll('.reveal').forEach(function (el) { el.classList.add('visible'); }); }
  });
})();